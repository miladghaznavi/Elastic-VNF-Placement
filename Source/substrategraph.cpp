...
Arc SubstrateGraph::arc(Node src, Node trg) {
    Arc re = INVALID;
    for (SubstrateGraph::OutArcIt a(*this, src); a != INVALID; ++a) {
        if (target(a) == trg) {
            re = a;
        }//if
    }//for
...
Arc SubstrateGraph::oposArc(Arc a) {
    return arc(target(a), source(a));
}
...
void SubstrateGraph::allocate(Node n, Capacity amount) {
    if (amount > residual(n)) {
        std::stringstream ss;
        ss  << "Cannot allocate more than residual capacity of node "
            << id(n)
            << ". Requested allocation is '"
...
void SubstrateGraph::allocate(Edge e, Capacity amount) {
    if (amount > residual(e)) {
        std::stringstream ss;
        ss  << "Cannot allocate more than residual capacity of edge "
            << id(e)
            << ". Requested allocation is '"
...
void SubstrateGraph::allocate(Arc a, Capacity amount) {
    if (amount > residual(a)) {
        std::stringstream ss;
        ss  << "Cannot allocate more than residual capacity of arc "
            << id(a)
            << ". Requested allocation is '"
...
void SubstrateGraph::release(Node n, Capacity release) {
    if (allocated(n) < release) {
        std::stringstream ss;
        ss  << "Cannot release more than what is allocated for node "
            << id(n)
            << ". Requested release is '"
...
void SubstrateGraph::release(Edge e,  Capacity release) {
    if (allocated(e) < release) {
        std::stringstream ss;
        ss  << "Cannot release more than what is allocated for edge "
            << id(e)
            << ". Requested release is '"
...
void SubstrateGraph::release(Arc a,  Capacity release) {
    if (allocated(a) < release) {
        std::stringstream ss;
        ss  << "Cannot release more than what is allocated for arc "
            << id(a)
            << ". Requested release is '"
