...
public:
    ...
    class EdgeRangeIt : public Edge {
        Identifier _end;
        const KAryFatTree* _graph;
        
    public:
        EdgeRangeIt(Invalid i) : Edge(i) { }
        
        EdgeRangeIt(const KAryFatTree& graph, const Edge& begin, const Edge& end) :
        _graph(&graph), Edge(begin) {
            this->_end = _graph->id(end);
        }
        
        EdgeRangeIt& operator++() {
            //Save _end pointer
            auto end = _end;
            _graph->next(*this);
            //restor _end pointer
    ...
    EdgeRangeIt coreLinkIt();
    EdgeRangeIt aggLinkIt ();
    EdgeRangeIt edgeLinkIt();
