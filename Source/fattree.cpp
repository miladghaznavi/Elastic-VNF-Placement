...
KAryFatTree::EdgeRangeIt KAryFatTree::coreLinkIt() {
    return EdgeRangeIt(*this,
                       this->edgeFromId(coreLinkNum() - 1),
                       Edge(Invalid()));
}

KAryFatTree::EdgeRangeIt KAryFatTree::aggLinkIt () {
    return EdgeRangeIt(*this,
                       this->edgeFromId(coreLinkNum() + aggLinkNum() - 1),
                       this->edgeFromId(coreLinkNum()));
}

KAryFatTree::EdgeRangeIt KAryFatTree::edgeLinkIt() {
    return EdgeRangeIt(*this,
                       this->edgeFromId(coreLinkNum() + aggLinkNum() + edgeLinkNum() - 1),
                       this->edgeFromId(coreLinkNum() + aggLinkNum()));
}
...
bool KAryFatTree::isHost(Node& n) {
    return (id(n) >= id(firstHost()) && id(n) <= id(lastHost()));
}

Edge KAryFatTree::hostEdge(Node& n) {
    if (!isHost(n)) {
        std::stringstream ss;
        ss << "Node " << id(n) << " is not a host!";
        throw std::invalid_argument(ss.str());
    }//if

    Edge e = IncEdgeIt(*this, n);
